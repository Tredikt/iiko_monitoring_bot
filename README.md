...
7. **db/repo.py** - хранение настроек в памяти:
   - Класс InMemorySettings для хранения настроек
   - Репозиторий SettingsRepo для работы с настройками
...
- Ежедневные отчёты с алертами при падении выручки
- Настройки хранятся в памяти (сбрасываются при перезапуске)
